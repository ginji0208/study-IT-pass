...
// アプリ起動時にカードをシャッフルするshuffleArray(cards);

let currentCardIndex = 0; // 現在表示しているカードのインデックス

// カードの内容を更新する関数
function updateCard() {
    flashcard.classList.remove('flipped'); // めくられている状態を解除
    questionElement.textContent = cards[currentCardIndex].question;
    answerElement.textContent = cards[currentCardIndex].answer;
}

// カードをめくる関数
flashcard.addEventListener('click', () => {
    flashcard.classList.toggle('flipped'); // 'flipped'クラスを付け外しする
});
